...
BEGIN_MESSAGE_MAP(CHDrawDoc, CDocument)
	...
	ON_COMMAND(ID_BUTTON_PIC, OnTestPic)
	...
	ON_UPDATE_COMMAND_UI(ID_BUTTON_PIC, OnUpdateBtPic)
...
{
	pCmdUI->SetCheck(PIC == m_picType);
}
void CHDrawDoc::OnUpdateArrow(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(PIC_ARROW == m_picType);
}
